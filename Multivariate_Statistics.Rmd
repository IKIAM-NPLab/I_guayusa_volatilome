...
scores <- ei_pca$x %>%                   # Get PC coordinates
  ...
  left_join(pheno_data )                 # Adding metadata
# Change the order of legend column of DataFrame
scores$Level_in_subQC <- factor(scores$Level_in_subQC,
                 levels = c("Alto Pano",
                            "Alto Tena",
                            "Talag",
                            "Early",
                 ...
                 labels=c("Alto Pano",
                            ...
                            "QC"))
# PCA plot
figure_s1e <- ggplot(scores,
                     aes(PC1, PC2, shape = Level_in_subQC,
                         color = Level_in_subQC)) +
  scale_color_manual(values=c("#00B0F6",
                              ...
                              "#7CAE00",
                              "#00BFC4")) +
  scale_shape_manual(values=c(17, 17, 17, 16, 16, 16, 8, 8, 3, 15)) + 
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (15.52 %)"),
         y=guide_axis(title = "PC2 (13.45 %)")) +
  labs(shape = 'SubQC', color= 'SubQC') +
  theme_classic() +
  theme(legend.position = c(0.88, 0.77),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(),
...
feat_data <- no_flag[!is.na(no_flag@featureData@data$Metabolite),]
# Extracting feature data
feat_data <- feat_data@featureData@data
# Creating a new small table with the annotated compounds
ei_compouds <- left_join(feat_data, loadings)
# Plotting results
figure_s1d <- ggplot(loadings, aes(PC1, PC2)) +
  geom_point(alpha = 0.2) +
  theme_classic() + 
  geom_point(data = ei_compouds,
             aes(shape = feat_data$Identification_level,
                 color = feat_data$Identification_level),
             size = 2) +
  labs(shape = 'Identification level',
       color = 'Identification level') +
  ggrepel::geom_label_repel(data = ei_compouds,
                            aes(label = feat_data$Metabolite),
                            box.padding = 0.37,
                            label.padding = 0.22,
                            label.r = 0.3,
                            cex = 2.5,
  ...
  guides(x=guide_axis(title = "PC1 (15.52 %)"),
         y=guide_axis(title = "PC2 (13.45 %)")) +
  theme(legend.position = c(0.945, 0.923),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(fill= "transparent")) +
  geom_vline(xintercept = 0, linetype = "longdash", colour="gray") +
...
# PCA plot
figure_1a <- ggplot(scores1,
                    aes(PC1, PC2, shape = Light_in_samples,
                        color = Light_in_samples)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (17.41 %)"),
         y=guide_axis(title = "PC2 (14.19 %)")) +
...
scores1 <- ei_pca1$x %>%                 # Get PC coordinates
  ...
  left_join(pheno_data1)                 # Adding metadata
# PCA plot
figure_1b <- ggplot(scores1,
                    aes(PC1, PC2, shape = Age_in_samples,
                        color = Age_in_samples)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (17.41 %)"),
         y=guide_axis(title = "PC2 (14.19 %)")) +
...
scores1 <- ei_pca1$x %>%                 # Get PC coordinates
  ...
  left_join(pheno_data1)                 # Adding metadata
# PCA plot
figure_1c <- ggplot(scores1,
                    aes(PC1, PC2, shape = Location_in_samples,
                        color = Location_in_samples)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (17.41 %)"),
         y=guide_axis(title = "PC2 (14.19 %)")) +
...
# Extracting feature identified
feat_data1 <- no_flag1[!is.na(no_flag1@featureData@data$Metabolite),]
# Extracting feature data
feat_data1 <- feat_data1@featureData@data
# Creating a new small table with the annotated compounds
ei_compouds1 <- left_join(feat_data1, loadings1)
# Plotting results
figure_1d <- ggplot(loadings1, aes(PC1, PC2)) +
  geom_point(alpha = 0.2) +
  theme_classic() + 
  geom_point(data = ei_compouds1,
             aes(shape = feat_data1$Identification_level,
                 color = feat_data1$Identification_level),
             size = 2) +
  labs(shape = 'Identification level',
       color = 'Identification level') +
  ggrepel::geom_label_repel(data = ei_compouds1,
                            aes(label = feat_data1$Metabolite),
                            box.padding = 0.37,
                            label.padding = 0.22,
                            label.r = 0.30,
                            cex = 2.5,
...
figure_2a
# Save plot
#ggsave('Result/notame_Result/figure_2a.pdf',
#       width = 5, height = 4, device='pdf', dpi="print")
...
figure_2c
# Save plot
#ggsave('Result/notame_Result/figure_2c.pdf',
#       width = 5, height = 4, device='pdf', dpi="print")
...
InChI_Keys <- c("NOORESDHJXLGAO-AATRIKPKSA-N",
...
# Exporting (*.png) file
ggsave(filename = "Result/notame_Result/figure_1.png", plot = figure_sone,
      width = 175, height = 120, units = "mm", dpi = 300, scale = 2.5)
# Exporting (*.jpg) file
...
ggsave(filename = "Result/notame_Result/figure_s1.pdf", plot = figure_sone,
      width = 140, height = 90, units = "mm", dpi = 300, scale = 2.5)
# Exporting (*.png) file
