...
write_to_excel(corrected_no_blank,
               "Result/notame_Result/corrected_no_blank.xlsx")
...
ggplot(scores,
  ...
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (22.98 %)"),
         y=guide_axis(title = "PC2 (12.28 %)")) +
...
# PCA scores
scores <- ei_pca1$x %>%                   # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
...
ggplot(scores,
  ...
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (24.68 %)"),
         y=guide_axis(title = "PC2 (12.29 %)")) +
  labs(shape = 'Light condition', color= 'Light condition') +
  theme_classic() +
  theme(legend.position = c(0.1, 0.18),
        legend.background = element_rect(fill = "white", color = "black")) +
...
# Centering and Scaling features
ei_pca2 <- prcomp(transp_table2, center = TRUE, scale. = FALSE)
...
# PCA scores
scores <- ei_pca2$x %>%                  # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
...
ggplot(scores,
  ...
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (34.58 %)"),
         y=guide_axis(title = "PC2 (17.99 %)")) +
...
# Centering and Scaling features
ei_pca3 <- prcomp(transp_table3, center = TRUE, scale. = FALSE)
...
# PCA scores
scores <- ei_pca3$x %>%                  # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
...
ggplot(scores,
       aes(PC1, PC2, shape = Age_in_samples, color = Light_in_samples)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (24.78 %)"),
         y=guide_axis(title = "PC2 (17.74 %)")) +
  labs(shape = 'Period age', color='Light condition') +
...
# Centering and Scaling features
ei_pca4 <- prcomp(transp_table4, center = TRUE, scale. = FALSE)
...
# PCA scores
scores <- ei_pca4$x %>%                  # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
...
ggplot(scores,
       aes(PC1, PC2, shape = Age_in_samples, color = Light_in_samples)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (33.09 %)"),
         y=guide_axis(title = "PC2 (18.69 %)")) +
  labs(shape = 'Period age', color='Light condition') +
  theme_classic() +
  theme(legend.position = c(0.1, 0.18),
        legend.background = element_rect(fill = "white", color = "black")) +
